...
class GCN_TransH_Model(tf.keras.Model):
    def __init__(self, num_entities, num_relations, embedding_dim, num_heads):
        ...
        # TransH-like embeddings
        self.transH_normal_vector = tf.keras.layers.Embedding(
            num_relations, embedding_dim, name='transH_normal_vector')
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        # TransH-like embeddings
        p_normal_vector = self.transH_normal_vector(inputs[:, 1])

        # Projection of entity embeddings onto relation-specific hyperplanes
        s_embed = s_embed - tf.reduce_sum(s_embed * p_normal_vector, axis=1, keepdims=True) * p_normal_vector
        o_embed = o_embed - tf.reduce_sum(o_embed * p_normal_vector, axis=1, keepdims=True) * p_normal_vector
...
# Create and compile the model
model = GCN_TransH_Model(num_entities, num_relations, embedding_dim, num_heads=2)
