...
class RotatE_Model(tf.keras.Model):
    def __init__(self, num_entities, num_relations, embedding_dim):
        ...
        self.entity_embeddings = tf.keras.layers.Embedding(
            num_entities, embedding_dim * 2, name='entity_embedding')  # Double the embedding dimensions for RotatE
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        # Convert RotatE embeddings from complex to real space
        s_embed_real = s_embed[:, :s_embed.shape[1] // 2]
        s_embed_imag = s_embed[:, s_embed.shape[1] // 2:]
        s_embed = tf.complex(s_embed_real, s_embed_imag)

        p_embed_real = p_embed
        p_embed_imag = tf.zeros_like(p_embed)
        p_embed = tf.complex(p_embed_real, p_embed_imag)

        o_embed_real = o_embed[:, :o_embed.shape[1] // 2]
        o_embed_imag = o_embed[:, o_embed.shape[1] // 2:]
        o_embed = tf.complex(o_embed_real, o_embed_imag)

        # Compute circular correlation
        predicted_o_embed = tf.signal.ifft(tf.signal.fft(s_embed) * tf.signal.fft(p_embed))
        predicted_o_embed = tf.math.real(predicted_o_embed)

        # Convert all embeddings back to real space for concatenation
        s_embed = tf.concat([tf.math.real(s_embed), tf.math.imag(s_embed)], axis=1)
        p_embed = tf.concat([tf.math.real(p_embed), tf.math.imag(p_embed)], axis=1)
        o_embed = tf.concat([tf.math.real(o_embed), tf.math.imag(o_embed)], axis=1)
        predicted_o_embed = tf.math.real(predicted_o_embed)
...
def lr_scheduler(epoch, lr):
    if epoch % 10 == 0 and epoch > 0:
        return lr * 0.9  # You can adjust the decay factor as needed
    return lr
