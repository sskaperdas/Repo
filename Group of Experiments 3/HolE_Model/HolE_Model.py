...
class HoIE_Model(tf.keras.Model):
    def __init__(self, num_entities, num_relations, embedding_dim):
        super(HoIE_Model, self).__init__()
    ...
    def hole_embedding(self, s_embed, p_embed, o_embed):
        # HolE specific operations
        circular_correlation = tf.reduce_sum(s_embed * o_embed * p_embed, axis=1)
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        # HolE specific embedding operation
        circular_correlation = self.hole_embedding(s_embed, p_embed, o_embed)

        # Reshape circular_correlation to be a 2D tensor
        circular_correlation = tf.reshape(circular_correlation, (-1, 1))

        # Combine HolE embedding with flattened output
        combined_output = tf.concat([flattened, circular_correlation], axis=1)

        prediction = self.dense(combined_output)
...
model = HoIE_Model(num_entities, num_relations, embedding_dim)
