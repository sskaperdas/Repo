...
class GCNHoIE_Model(tf.keras.Model):
    def __init__(self, num_entities, num_relations, embedding_dim, num_heads):
        ...
        self.hoie_embeddings = tf.keras.layers.Embedding(
            num_relations, embedding_dim * 2, name='hoie_embedding')
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        # HoIE embeddings
        hoie_embed = self.hoie_embeddings(inputs[:, 1])

        s_embed = s_embed + hoie_embed[:, :embedding_dim]
        o_embed = o_embed + hoie_embed[:, embedding_dim:]
...
# Create and compile the model
model = GCNHoIE_Model(num_entities, num_relations, embedding_dim, num_heads=2)
