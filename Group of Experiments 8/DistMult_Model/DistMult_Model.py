...
# Define the triple classification DistMult model with the Conv1D architecture
class Complex_Model_With_DistMult(tf.keras.Model):
    def __init__(self, num_entities, num_relations, embedding_dim, dropout_rate=0.5, l2_reg=0.01):
        super(Complex_Model_With_DistMult, self).__init__()
        ...
        self.gcn_layer1 = GCNLayer(units=embedding_dim, activation='relu', use_bias=True, kernel_regularizer='l2')
        self.gcn_layer2 = GCNLayer(units=embedding_dim, activation='relu', use_bias=True, kernel_regularizer='l2')
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        # First GCN layer
        s_embed = self.gcn_layer1(s_embed)
        o_embed = self.gcn_layer1(o_embed)

        # Second GCN layer
        s_embed = self.gcn_layer2(s_embed)
        o_embed = self.gcn_layer2(o_embed)

        # DistMult scoring
        scores = tf.reduce_sum(s_embed * p_embed * o_embed, axis=-1)
...
# Create and compile the model
model = Complex_Model_With_DistMult(num_entities, num_relations, embedding_dim)
