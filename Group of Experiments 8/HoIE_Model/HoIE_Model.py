...
# Define the triple classification HoIE model with the Conv1D architecture
class GCN_Complex_Model_With_HoIE(tf.keras.Model):
    def __init__(self, num_entities, num_relations, embedding_dim, hoie_embedding_dim, dropout_rate=0.5, l2_reg=0.01):
        super(GCN_Complex_Model_With_HoIE, self).__init__()
        ...
        # GCN Layers
        self.gcn_layer1 = GCNLayer(units=embedding_dim, activation='relu', name='gcn_layer1', kernel_regularizer='l2',
                                   use_bias=True)
        self.gcn_layer2 = GCNLayer(units=embedding_dim, activation='relu', name='gcn_layer2', kernel_regularizer='l2',
                                   use_bias=True)
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        # GCN layers
        s_embed_gcn = self.gcn_layer1(s_embed)
        o_embed_gcn = self.gcn_layer2(o_embed)

        # Skip connections
        s_embed_gcn_skip = tf.concat([s_embed, s_embed_gcn], axis=-1)
        o_embed_gcn_skip = tf.concat([o_embed, o_embed_gcn], axis=-1)

        # Modify the concatenation to include hoIE embeddings
        concatenated_embed = tf.concat([s_embed_gcn_skip, p_embed, o_embed_gcn_skip, s_hoie, o_hoie], axis=1)
...
# Model checkpoint callback to save the model after each epoch
model_checkpoint = ModelCheckpoint('ComplexModelWithHoIEGCN', save_best_only=True)

# Create and compile the model
model = GCN_Complex_Model_With_HoIE(num_entities, num_relations, embedding_dim, embedding_dim)
...
# Train the model
history = model.fit(
    np.array(X_train), np.expand_dims(y_train, axis=1),  # Ensure labels have the correct shape
    epochs=num_epochs,
    batch_size=batch_size,
    validation_data=(np.array(X_val), np.expand_dims(y_val, axis=1)),  # Ensure validation labels have the correct shape
