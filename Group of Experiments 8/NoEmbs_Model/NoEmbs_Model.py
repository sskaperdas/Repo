...
# Define the triple classification NoEmbds link prediction model with a simple graph convolutional layer
class GCN_NoEmbds_Model(tf.keras.Model):
    def __init__(self, num_entities, num_relations, embedding_dim, dropout_rate=0.5, l2_reg=0.01):
        ...
        self.gcn_layer1 = GCNLayer(embedding_dim, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(l2_reg))
        self.gcn_layer2 = GCNLayer(embedding_dim, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(l2_reg))
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        # Graph Convolution Layers
        p_embed = self.gcn_layer1(p_embed)
        p_embed = self.gcn_layer2(p_embed)
