...
# Define the triple classification RotatE model with the dense architecture
class RotatE_Model(tf.keras.Model):
    def __init__(self, num_entities, num_relations, embedding_dim):
        super(RotatE_Model, self).__init__()
        ...
        # Add a scaling factor for RotatE
    ...
    def call(self, inputs, training=None, mask=None):
        # Separate real and imaginary parts for entities and relations
        ...
        # Combine RotatE embeddings (normalize entities)
        s_embed_modulus = tf.sqrt(tf.square(s_embed_real) + tf.square(s_embed_imag))
        s_embed_real_normalized = s_embed_real / s_embed_modulus
        s_embed_imag_normalized = s_embed_imag / s_embed_modulus
        s_embed = tf.concat([s_embed_real_normalized, s_embed_imag_normalized], axis=-1)

        p_embed_modulus = tf.sqrt(tf.square(p_embed_real) + tf.square(p_embed_imag))
        p_embed_real_normalized = p_embed_real / p_embed_modulus
        p_embed_imag_normalized = p_embed_imag / p_embed_modulus
        p_embed = tf.concat([p_embed_real_normalized, p_embed_imag_normalized], axis=-1)

        o_embed_modulus = tf.sqrt(tf.square(o_embed_real) + tf.square(o_embed_imag))
        o_embed_real_normalized = o_embed_real / o_embed_modulus
        o_embed_imag_normalized = o_embed_imag / o_embed_modulus
        o_embed = tf.concat([o_embed_real_normalized, o_embed_imag_normalized], axis=-1)

        # Entity degree embeddings
        in_degree_embed = self.in_degree(inputs[:, 0])
        out_degree_embed = self.out_degree(inputs[:, 2])

        # Relation frequency embedding
        relation_freq_embed = self.inverse_relation_frequency_embedding(inputs[:, 1])

        concat_embed = tf.concat([s_embed_real, s_embed_imag, p_embed_real, p_embed_imag, o_embed_real,
                                  o_embed_imag, in_degree_embed, out_degree_embed, relation_freq_embed], axis=-1)
...
X_train_val, X_test, y_train_val, y_test = train_test_split(all_triples_with_degrees, labels, test_size=0.2,
...
for fold, (train_index, val_index) in enumerate(stratified_kfold.split(X_train_val, y_train_val)):
    ...
    model = RotatE_Model(num_entities, num_relations, embedding_dim)
