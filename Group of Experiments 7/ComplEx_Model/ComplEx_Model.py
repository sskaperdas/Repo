...
# Define the triple classification ComplEx model with the Conv1D architecture
class ComplEx_Model(tf.keras.Model):
    def __init__(self, num_entities, num_relations, embedding_dim, dropout_rate=0.5, l2_reg=0.01):
        ...
        self.entity_real_embeddings = tf.keras.layers.Embedding(num_entities, embedding_dim, name='entity_real_embedding')
        self.entity_imag_embeddings = tf.keras.layers.Embedding(num_entities, embedding_dim, name='entity_imag_embedding')
        self.relation_real_embeddings = tf.keras.layers.Embedding(num_relations, embedding_dim, name='relation_real_embedding')
        self.relation_imag_embeddings = tf.keras.layers.Embedding(num_relations, embedding_dim, name='relation_imag_embedding')
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        s_real_embed = self.entity_real_embeddings(s_idx)
        s_imag_embed = self.entity_imag_embeddings(s_idx)
        p_real_embed = self.relation_real_embeddings(p_idx)
        p_imag_embed = self.relation_imag_embeddings(p_idx)

        # ComplEx embeddings
        s_embed = tf.concat([s_real_embed, s_imag_embed], axis=1)
        p_embed = tf.concat([p_real_embed, p_imag_embed], axis=1)

        concatenated_embed = tf.concat([s_embed, p_embed], axis=1)
