...
class Complex_Model_With_HoIE_GAT_GCN(tf.keras.Model):
    def __init__(self, num_entities, num_relations, embedding_dim, hoie_embedding_dim, dropout_rate=0.5, l2_reg=0.01):
        super(Complex_Model_With_HoIE_GAT_GCN, self).__init__()

        self.entity_embeddings = tf.keras.layers.Embedding(num_entities, embedding_dim, name='entity_embedding')
        self.hoie_embeddings = tf.keras.layers.Embedding(num_entities, hoie_embedding_dim * 2, name='hoie_embedding')  # *2 for real and imaginary parts
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        concatenated_embed = tf.concat([s_embed, p_embed, o_embed, s_hoie, o_hoie, s_gcn, p_gcn, o_gcn, s_gat, p_gat, o_gat], axis=1)
...
# Create and compile the model
model = Complex_Model_With_HoIE_GAT_GCN(num_entities, num_relations, embedding_dim, hoie_embedding_dim=embedding_dim)
