...
class GCN_GAT_NoEmbds_Model(tf.keras.Model):
    def __init__(self, num_entities, num_relations, embedding_dim, dropout_rate=0.5, l2_reg=0.01):
        ...
        self.conv1d_1 = tf.keras.layers.Conv1D(128, 3, activation='relu', padding='same', kernel_regularizer=tf.keras.regularizers.l2(l2_reg))
        self.batch_norm1 = tf.keras.layers.BatchNormalization()
        self.dropout1 = tf.keras.layers.Dropout(dropout_rate)

        self.conv1d_2 = tf.keras.layers.Conv1D(256, 3, activation='relu', padding='same', kernel_regularizer=tf.keras.regularizers.l2(l2_reg))
        self.batch_norm2 = tf.keras.layers.BatchNormalization()
        self.dropout2 = tf.keras.layers.Dropout(dropout_rate)

        # GCN and GAT layers
        self.gcn_layer = GCNLayer(units=256, activation='relu', kernel_regularizer='l2')
        self.gat_layer = GATLayer(num_heads=4, output_dim=256, activation='relu', dropout_rate=0.3)
        ...
        self.dense2 = tf.keras.layers.Dense(1, activation='sigmoid', kernel_regularizer=tf.keras.regularizers.l2(l2_reg))
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        gat_output = self.gat_layer(x)

        # Concatenate GCN and GAT outputs
        graph_output = tf.concat([gcn_output, gat_output], axis=1)

        x = self.flatten(graph_output)
...
# Create and compile the model
model = GCN_GAT_NoEmbds_Model(num_entities, num_relations, embedding_dim)
