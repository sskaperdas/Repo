...
class ModelWithRotatEAndGCNAndGAT(tf.keras.Model):
    def __init__(self, num_entities, num_relations, embedding_dim, rot_embedding_dim, dropout_rate=0.5, l2_reg=0.01, gcn_units=512, gat_num_heads=8):
        super(ModelWithRotatEAndGCNAndGAT, self).__init__()
        ...
        self.gcn_layer = GCNLayer(units=gcn_units, activation='relu', use_bias=True)

        # Graph Attention Layer
        self.gat_layer = GATLayer(num_heads=gat_num_heads, output_dim=embedding_dim*2, activation='relu', dropout_rate=dropout_rate)
...
# Create and compile the model
model = ModelWithRotatEAndGCNAndGAT(num_entities, num_relations, embedding_dim, embedding_dim)
