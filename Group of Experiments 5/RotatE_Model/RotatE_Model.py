...
class RotatEEncoder(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.imaginary_weights = self.add_weight(
            shape=(self.embedding_dim,),
            initializer='glorot_uniform',
            trainable=True,
            name='imaginary_weights'
        )

    def call(self, inputs):
        entity_real = inputs[:, :self.embedding_dim]
        entity_imaginary = inputs[:, self.embedding_dim:]
        rotated_real = entity_real * K.cos(self.real_weights) - entity_imaginary * K.sin(self.real_weights)
        rotated_imaginary = entity_real * K.sin(self.real_weights) + entity_imaginary * K.cos(self.real_weights)
        return tf.concat([rotated_real, rotated_imaginary], axis=-1)


class GAT_RotatE_Model(tf.keras.Model):
    def __init__(self, num_entities, num_relations, embedding_dim, num_heads, dropout_rate=0.3):
        super(GAT_RotatE_Model, self).__init__()

        self.entity_embeddings = tf.keras.layers.Embedding(
            num_entities, 2 * embedding_dim, name='entity_embedding')
        self.relation_embeddings = tf.keras.layers.Embedding(
            num_relations, 2 * embedding_dim, name='relation_embedding')

        self.entity_encoder = RotatEEncoder(embedding_dim)
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        # Encode entity embeddings using RotatE
        s_embed = self.entity_encoder(s_embed)
        o_embed = self.entity_encoder(o_embed)

        # Compute the circular correlation of entity and relation embeddings
        complex_multiply = tf.reduce_sum(s_embed * p_embed * tf.math.conj(o_embed), axis=-1, keepdims=True)

        # Apply dropout
        x = self.dropout(complex_multiply, training=training)
...
model = GAT_RotatE_Model(num_entities, num_relations, embedding_dim, num_heads=4, dropout_rate=0.5)
