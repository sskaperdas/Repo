...
class GAT_ComplEx_Model(tf.keras.Model):
    def __init__(self, num_entities, num_relations, embedding_dim, num_heads, dropout_rate=0.3):
        ...
        # Add embedding layers for entity degree and inverse relation frequency
        self.in_degree = tf.keras.layers.Embedding(
            num_entities, embedding_dim, name='in_degree_embedding')
        self.out_degree = tf.keras.layers.Embedding(
            num_entities, embedding_dim, name='out_degree_embedding')
        self.relation_frequency = tf.keras.layers.Embedding(
            num_relations, embedding_dim, name='relation_frequency')
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        # Add embedding lookups for entity degree and inverse relation frequency
        in_degree_embed = self.in_degree(inputs[:, 0])
        out_degree_embed = self.out_degree(inputs[:, 2])
        relation_freq_embed = self.relation_frequency(inputs[:, 1])

        # Combine ComplEx embeddings
        s_embed = tf.concat([s_embed_real, s_embed_imag, in_degree_embed], axis=-1)
        p_embed = tf.concat([p_embed_real, p_embed_imag, relation_freq_embed], axis=-1)
        o_embed = tf.concat([o_embed_real, o_embed_imag, out_degree_embed], axis=-1)
...
model = GAT_ComplEx_Model(num_entities, num_relations, embedding_dim, num_heads=4, dropout_rate=0.5)
