...
class GAT_TransH_Model(tf.keras.Model):
    def __init__(self, num_entities, num_relations, embedding_dim, num_heads, dropout_rate=0.3):
        super(GAT_TransH_Model, self).__init__()

        self.entity_embeddings_entity_space = tf.keras.layers.Embedding(
            num_entities, embedding_dim, name='entity_embedding_entity_space')
        self.entity_embeddings_hyperplane_space = tf.keras.layers.Embedding(
            num_entities, embedding_dim, name='entity_embedding_hyperplane_space')
        self.relation_embeddings_entity_space = tf.keras.layers.Embedding(
            num_relations, embedding_dim, name='relation_embedding_entity_space')
        self.relation_embeddings_hyperplane_space = tf.keras.layers.Embedding(
            num_relations, embedding_dim, name='relation_embedding_hyperplane_space')
    ...
    def call(self, inputs, training=None, mask=None):
        s_embed_entity_space = self.entity_embeddings_entity_space(inputs[:, 0])
        s_embed_hyperplane_space = self.entity_embeddings_hyperplane_space(inputs[:, 0])
        p_embed_entity_space = self.relation_embeddings_entity_space(inputs[:, 1])
        p_embed_hyperplane_space = self.relation_embeddings_hyperplane_space(inputs[:, 1])
        o_embed_entity_space = self.entity_embeddings_entity_space(inputs[:, 2])
        o_embed_hyperplane_space = self.entity_embeddings_hyperplane_space(inputs[:, 2])

        # Calculate the projected embeddings
        s_projected = s_embed_entity_space - tf.reduce_sum(s_embed_entity_space * p_embed_entity_space, axis=1,
                                                           keepdims=True) * p_embed_hyperplane_space
        o_projected = o_embed_entity_space - tf.reduce_sum(o_embed_entity_space * p_embed_entity_space, axis=1,
                                                           keepdims=True) * p_embed_hyperplane_space

        # Concatenate the embeddings
        concat_embed = tf.concat([s_projected, p_embed_entity_space, o_projected], axis=1)
...
model = GAT_TransH_Model(num_entities, num_relations, embedding_dim, num_heads=4, dropout_rate=0.5)
