...
class GCN_GAT_DistMult_Model(tf.keras.Model):
    def __init__(self, num_entities, num_relations, embedding_dim, num_heads, dropout_rate=0.3):
        super(GCN_GAT_DistMult_Model, self).__init__()
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        # Apply dropout
        s_embed = self.dropout(s_embed, training=training)
        p_embed = self.dropout(p_embed, training=training)
        o_embed = self.dropout(o_embed, training=training)

        # Apply GCN layers
        for gcn_layer in self.gcn_layers:
            s_embed = gcn_layer(s_embed)
            p_embed = gcn_layer(p_embed)
            o_embed = gcn_layer(o_embed)

        # Apply GAT layers
        for gat_layer in self.gat_layers:
            s_embed = gat_layer(s_embed)
            p_embed = gat_layer(p_embed)
            o_embed = gat_layer(o_embed)

        # Apply dense layers
        for dense_layer in self.dense_layers:
            s_embed = dense_layer(s_embed)
            p_embed = dense_layer(p_embed)
            o_embed = dense_layer(o_embed)

        # Calculate scores using DistMult
        scores = tf.reduce_sum(s_embed * p_embed * o_embed, axis=1, keepdims=True)

        # Final dense layer for prediction
        prediction = self.final_dense(scores)
...
model = GCN_GAT_DistMult_Model(num_entities, num_relations, embedding_dim, num_heads=4, dropout_rate=0.5)
