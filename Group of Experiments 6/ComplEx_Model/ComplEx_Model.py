...
class GCN_GAT_Model_ComplEx(tf.keras.Model):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        transformed_o_embed_real = s_embed_real * p_embed_real - s_embed_imag * p_embed_imag
        transformed_o_embed_imag = s_embed_real * p_embed_imag + s_embed_imag * p_embed_real

        # Concatenate the real and imaginary parts of the embeddings
        concat_embed_real = tf.concat([s_embed_real, transformed_o_embed_real, o_embed_real], axis=1)
        concat_embed_imag = tf.concat([s_embed_imag, transformed_o_embed_imag, o_embed_imag], axis=1)

        # Combine real and imaginary parts to form the final complex embeddings
        concat_embed = tf.concat([concat_embed_real, concat_embed_imag], axis=1)
...
model = GCN_GAT_Model_ComplEx(num_entities, num_relations, embedding_dim, num_heads=4, dropout_rate=0.5)
