...
class Attention(tf.keras.layers.Layer):
    ...
    def call(self, query, values):
        ...
        score = self.V(tf.nn.tanh(query_with_time_axis + self.W_key(values)))
...
class GCN_HoIE_Model_With_Improved_Attention(tf.keras.Model):
    def __init__(self, num_entities, num_relations, embedding_dim, hoie_embedding_dim, gcn_units, dropout_rate=0.5,
                 l2_reg=0.01):
        super(GCN_HoIE_Model_With_Improved_Attention, self).__init__()
        ...
        self.attention = Attention(gcn_units)
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        context_vector, _ = self.attention(gcn_output2, gcn_output2)
...
# Create and compile the model
model = GCN_HoIE_Model_With_Improved_Attention(num_entities, num_relations, embedding_dim, embedding_dim,
                                               gcn_units=embedding_dim)
