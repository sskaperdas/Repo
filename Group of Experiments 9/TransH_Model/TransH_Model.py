...
# Define the triple classification TransH model with the Conv1D architecture
class Complex_Model_With_TransH_and_GAT(tf.keras.Model):
    def __init__(self, num_entities, num_relations, embedding_dim, projection_dim, dropout_rate=0.5, l2_reg=0.01, num_heads=8, gat_output_dim=1024):
        super(Complex_Model_With_TransH_and_GAT, self).__init__()
        ...
        self.relation_projections = tf.keras.layers.Embedding(num_relations, projection_dim, name='relation_projection')

        # GAT Layer
        self.gat_layer = GATLayer(num_heads=num_heads, output_dim=gat_output_dim, dropout_rate=dropout_rate)
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        s_proj = s_embed - tf.reduce_sum(s_embed * p_proj, axis=-1, keepdims=True) * p_proj
        o_proj = s_proj + p_embed

        # TransE-like embeddings
        predicted_o_embed = s_embed + p_embed

        concatenated_embed = tf.concat([s_embed, p_embed, o_proj, predicted_o_embed], axis=1)

        # Apply GAT layer
        x = self.gat_layer(concatenated_embed, training=training)

        # Reshape tensor to match GAT layer's expected input shape
        x = tf.expand_dims(x, axis=1)  # Add batch dimension
...
# Create and compile the model
model = Complex_Model_With_TransH_and_GAT(num_entities, num_relations, embedding_dim, projection_dim=embedding_dim)
